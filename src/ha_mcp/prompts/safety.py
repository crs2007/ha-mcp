...
def register_safety_prompts(mcp: FastMCP) -> None:
    ...
    @mcp.prompt()
    async def confirm_action(action: str, entity_id: str) -> list[PromptMessage]:
    ...
    @mcp.prompt()
    async def quiet_hours_check(entity_id: str, purpose: str) -> list[PromptMessage]:
    ...
    @mcp.prompt()
    async def read_before_write(entity_id: str) -> list[PromptMessage]:
    ...
    @mcp.prompt()
    async def bulk_action_review(entities: str, action: str) -> list[PromptMessage]:
