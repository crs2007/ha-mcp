...
Replaces the full tool catalog (~89 tools) with search-based discovery (~4 proxy tools). When enabled, tools are found via `ha_search_tools` and executed through categorized proxies (read/write/delete).
...
### Assist
- `ha_intent_process` — Run a natural-language sentence through Home Assistant's Assist intent system.
